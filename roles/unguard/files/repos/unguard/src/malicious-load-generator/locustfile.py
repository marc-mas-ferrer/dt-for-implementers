...
LOCATION_BASED_IPS = ['204.176.161.159'] 
"""
                      '18.153.60.55',
                      '96.16.70.23',
                      '171.72.188.229',
...
class UnguardUser(HttpUser):
    ...
    def get_random_x_forwarded_for_header_CMD_INJ(self):
        return {
            'x-forwarded-for': random.choice(LOCATION_BASED_IPS_CMD_INJ)
        }
    ...
    def post_cmd(self):
        ...
        self.client.post("/post", data=cmd_post, headers=self.get_random_x_forwarded_for_header_CMD_INJ())
        time.sleep(1)
